...
1.  What does each row of the `country_inflation` dataset represent?
    ...
    Each row is a year, each column is a country
...
2.  Reshape (pivot) `country_inflation` such that each row represents a country/year combination, with columns `country`, `year`, and `annual_inflation`.
    ...
    country_inflation_tidy <- country_inflation |> pivot_longer(
      cols = -country,
      names_to = "year",
...
3.  Create a vector called `countries_of_interest` which contains the names of countries you want to visualize the inflation rates for over the years.
    ...
    ```{r}
    countries_of_interest <- c("Türkiye", "United States", "Australia")
    ```
...
country_inflation_tidy |>
  filter(country %in% countries_of_interest) |>
  ggplot(aes(x = year, y = annual_inflation, 
             group = country, color = country)) +
  geom_point() + geom_line() +
  scale_x_discrete(breaks = seq(1993, 2021, 3)) +
...
us_inflation |>
  ...
  geom_point() + geom_line() +
  labs(
    x = "Year", y = "Annual Inflation",
    title = "Annual Inflation in Sectors of Interest",
    color = "Sector"
  ) + 
  theme(
