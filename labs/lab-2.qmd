...
1.  How many observations and how many variables are in the dataset? Use inline code to answer this question. What does each row represent? There are r nrow(nobel) observations, and r ncol(nobel) variables in the dataset. (I have no idea why inline code is not working)
...
ggplot(nobel_living_science, aes(y = country_us)) + 
  ...
  facet_wrap(~category) + 
  labs(
    x = "Count", y = "Country (US/Not US)",
    title = "Are Nobel Laureates from the US?"
  )
```

The Buzzfeed article's claims are supported by the data.
...
The relative frequency bar plot better shows the split between laureates born in the US and not born in the US.
There are many laureates from the US that were not born in the US, but the majority of US laureates were born in the US.
