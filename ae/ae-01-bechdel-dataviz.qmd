...
::: callout-important
Go to the [course GitHub organization](https://github.com/sta199-f22-1) and locate the repo titled `ae-01-YOUR_GITHUB_USERNAME` to get started.
...
-   How many observations (rows) are in the data set? **1615**
...
ggplot(bechdel, 
  ...
  labs(
    ...
    title = "Gross revenue vs. budget, by passing/failing Bechdel"
    )
```
...
ggplot(bechdel, 
  ...
  labs( 
    x = "Budget (in 2013 $)", 
    y = "Gross revenue (in 2013 $)", 
    title = "Gross revenue vs. budget, by passing/failing Bechdel and ROI" )
```
...
ggplot(bechdel, 
       aes(x = budget_2013, y = gross_2013, 
           color = binary, size = roi, alpha = 0.5)) + 
  geom_point() + 
  labs( 
    x = "Budget (in 2013 $)", 
    y = "Gross revenue (in 2013 $)", 
...
ggplot(bechdel, 
  ...
  labs( 
    ...
    title = "Gross revenue vs. budget, by passing/failing Bechdel and ROI" ) + 
  facet_wrap(~clean_test) + 
  scale_x_continuous("Budget in 2013", 
                     breaks = scales::breaks_width(2e+08)) + 
  scale_y_continuous("Gross Revenue in 2013", 
                     breaks = scales::breaks_width(1e+09))
```
...
This update wasn't particularly useful because the clean_test variable already separates the data in the way that the binary variable does (PASS = ok, FAIL = everything else).
...
-   Got rid of redundant x/y labels in labs()

-   Changed figure size to get rid of overlap

```{r plot-improve, fig.height = 3, fig.width = 5}
#| label: plot-improve
...
ggplot(bechdel, 
       aes(x = budget_2013, y = gross_2013, 
           color = binary, size = roi, alpha = 0.5)) + 
  geom_point() + 
  labs(title = "Gross revenue vs. Budget, by Binary & ROI") + 
  facet_wrap(~clean_test) + 
  scale_x_continuous("Budget in 2013", 
                     breaks = scales::breaks_width(2e+08)) + 
  scale_y_continuous("Gross Revenue in 2013", 
...
Expand on your plot from the previous step to zoom in on movies with `roi < 100` to get a better view of how the medians across the categories compare.
...
bechdel |> 
  filter(roi < 100) |>
  ggplot(aes(x = clean_test, y = roi, color = binary)) + 
  geom_boxplot()
```
