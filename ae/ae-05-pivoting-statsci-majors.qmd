...
*More people are majoring in statsci at Duke, especially BS and BS2 degrees.*
...
For each observation, we need their graduation year and degree type.
...
statsci_pivot <- statsci |> pivot_longer(
  cols = starts_with("20"),
  names_to = "year",
  values_to = "n"
) |>
  mutate(year = as.integer(year),
         ...
         degree_type = str_extract(string = degree,
                                   pattern = "(?<=\\().*(?=\\))")) |>
  arrange(match(degree_type, c("BS", "BS2", "AB", "AB2")))
statsci_pivot
```
...
statsci_plot_draft <-
  ggplot(statsci_pivot, aes(x = year, y = n, color = degree_type)) + 
...
*Color, labels, scale, move legend*
...
statsci_plot_draft <- statsci_plot_draft + 
...
```{r finalfig, fig.width = 7, fig.height = 5}
#| label: plot-improve-4

statsci_plot_draft + theme_minimal() + 
  theme(
