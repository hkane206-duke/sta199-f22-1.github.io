...
fisheries |> summarize(mean_aq_prop = mean(aq_prop, na.rm = TRUE))
```
...
fisheries |> summarize(
  mean_aq_prop = mean(aq_prop, na.rm = TRUE),
...
fisheries_summary <- fisheries_summary |> arrange(desc(mean_aq_prop))
...
ggplot(fisheries_summary, aes(
  y = fct_reorder(continent, mean_aq_prop),
  x = mean_aq_prop
)) + 
  scale_x_continuous(lab = label_percent(accuracy = 1)) +
  geom_col() + 
  labs(x = NULL, y = NULL,
