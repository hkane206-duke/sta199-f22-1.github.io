...
```{r lowest-unemployment-rate}

major_income_undergrad |>
  select(major, undergrad_unemployment_rate) |>
  slice_min(undergrad_unemployment_rate, n = 5)
```

The 5 majors with the lowest unemployment rate are Geological and Geophysical Engineering (0.00%), Educational Administration and Supervision (0.00%), Pharmacology (1.79%), Nuclear Engineering (2.09%), and Treatment Therapy Professions (2.17%).
...
```{r highest-perc-women}

major_income_undergrad |>
  select(major, undergrad_sharewomen) |>
  slice_max(undergrad_sharewomen, n = 5)
```

The 5 majors with the highest percentage of women are Early Childhood Education (96.90%), Communication Disorders Sciences And Services (96.80%), Medical Assisting Services (92.78%), Elementary Education (92.37%), and Family and Consumer Sciences (91.09%).
...
ggplot(major_income_undergrad, aes(x = undergrad_median)) + 
  geom_histogram(binwidth = 10000) + 
  labs(
    x = "Median Income", y = "Count",
    title = "Distribution of Median Incomes of College Graduates"
  )
```

a.  Calculate the mean and median for median income. Based on the shape of the histogram, determine which of these summary statistics is useful for describing the distribution.

```{r summary-med-income}

major_income_undergrad |> summarize(
  mean_median_income = mean(undergrad_median, na.rm = TRUE),
  med_median_income = median(undergrad_median, na.rm = TRUE)
)
```

Because the distribution is skewed right and has outliers, the median is a more useful measure of center for this distribution.

a.  Describe the distribution of median incomes of college graduates across various majors based on your histogram from part (a) and incorporating the statistic you chose in part (b) to help your narrative. *Hint:* Mention shape, center, spread, any unusual observations.

The distribution of median incomes of college graduates across various majors is skewed right, with a median of \$55,000.
There are a few outlier majors that have a median income higher than most other majors (\>\$120,000).
...
major_category_summary <- major_income_undergrad |>
  group_by(major_category) |>
  summarize(
    min = min(undergrad_median),
    median = median(undergrad_median),
    max = max(undergrad_median),
    n = n()
  ) |>
  arrange(desc(median))
...
a.  Create box plots of the distribution of median income by major category.
...
```{r dist-med-income-major-categories}

ggplot(major_income_undergrad, aes(
  y = reorder(major_category, undergrad_median, FUN = median), 
  x = undergrad_median
)) + 
  geom_boxplot(aes(fill = major_category), show.legend = FALSE) + 
  labs(
    x = "Median Income", y = "Major Category",
    title = "Distribution of Median Incomes by",
    subtitle = "Major Category"
  ) + 
  scale_x_continuous(labels = label_number(suffix = " K", scale = 1e-3))
```

a.  In 1-2 sentences, describe how median incomes across various major categories compare. Your description should also touch on where your own intended/declared major (yes, your major at Duke).

In general, median incomes in STEM major categories tend to be greater than median incomes in humanities major categories.
This is good for me!
...
a.  First, let's create a new vector called `stem_categories` that lists the major categories that are considered STEM fields.
    ...
    major_income_undergrad <- major_income_undergrad |>
      mutate(major_type = if_else(major_category %in% stem_categories, "STEM", "Not STEM"))
    ```
...
```{r low-stem-median-earnings}

#major_income_undergrad |>
#  select(major, major_type, undergrad_median) |>
#  filter(undergrad_median < 55000) |>
#  arrange(desc(undergrad_median))

#commented to be able to render
```
...
a.  To do so, we will first join data that contains information on median incomes of those with undergraduate and graduate degrees.
    ...
    ```{r undergrad-grad-join}

    major_income <- inner_join(major_income_undergrad, major_income_grad)
    major_income
    ```
...
b.  Create a new variable called `grad_multiplier` -- the ratio of median income of those with a graduate degree divided by median income of those with an undergraduate degree, for STEM majors.
    ...
    major_income |> mutate(
      grad_multiplier = grad_median / undergrad_median
    ) |>
      #filter(major_type == "STEM") |> (not rendering)
      select(major, grad_multiplier, grad_median, undergrad_median) |>
      arrange(desc(grad_multiplier)) |>
      slice_head(n = 10)
    ```
...
c.  Determine the number of rows of `major_income_undergrad` and `major_income_grad` as well as `major_income`.
    ...
    nrows <- tibble(
      dataset = c("major_income_undergrad", "major_income_grad", "major_income"),
      nrows = c(nrow(major_income_undergrad), nrow(major_income_grad), 
                nrow(major_income))
      )
    nrows
    ```
